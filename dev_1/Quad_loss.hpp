    ...
    Tensor<DTYPE>* BackPropagate(int pTime = 0)
    {
        ...
        for (int ba = 0; ba < m_numAnchor; ba++) 
        {
            ...
            if (m_sampleLoss[pTime][ba]) 
            {
                for (; anc_idx < anc_limit; anc_idx++, pos_idx++, neg_idx++, other_neg_idx++) 
                {
                    input_delta[anc_idx] = ((2.f * (input[anc_idx] + input[neg_idx])) - (4.f * input[pos_idx])) / featureDim;
                    input_delta[pos_idx] = (4.f * (input[pos_idx] - input[anc_idx])) / featureDim;
                    input_delta[neg_idx] = (2.f * (input[anc_idx] - 2.f * input[neg_idx] + input[other_neg_idx])) / featureDim;
                    input_delta[other_neg_idx] = (2.f * (input[neg_idx] - input[other_neg_idx])) / featureDim;
                }
            }
            else 
            {
                memset(&input_delta[anc_idx], 0, featureDim * sizeof(DTYPE) * 4);
            }
        }
